   ...
   // }
    ...
    request.addEvent('complete', function (responseJSON) {
        if (content_type_undefined == 0) {
            ...
            // Bind the values based on resource type
            if(resource_type !='sitecrowdfunding_project'){
                if (responseJSON.favourite_id) {
                    ...
                    if ($(content_type + '_total_no_of_favourite_' + resource_id)) {
                        let idName= content_type + '_total_no_of_favourite_' + resource_id;
                        document.getElementById(idName).innerHTML = responseJSON.total_no_of_favourite;
                    }
            ...
            // Refresh the page if it project type
            if(resource_type == 'sitecrowdfunding_project'){
                var message;
                if(responseJSON.favType == 'UNFAVOURITE'){
