...
google.maps.event.addListener(autocompleteSECreateLocation, 'place_changed', function() {
  ...
  var types_location = '';
	...
	var locationParams = '{"location" :"' + document.getElementById('customLocation').value.replace(/\"/g, "") + '","latitude" :"' + place.geometry.location.lat() + '","longitude":"' + place.geometry.location.lng() + '","formatted_address":"' + place.formatted_address.replace(/\"/g, "") + '","address":"' + address.replace(/\"/g, "") + '","country":"' + country + '","state":"' + state + '","zipcode":"' + zipcode + '","city":"' + city + '"}';
  
	document.getElementById('customLocationParams').value = locationParams;
});
