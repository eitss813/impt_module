    ...
    <?php foreach( $this->navigation as $item ): ?>
      ...
      <li class="seaocore_mimi_menu_item seaocore_mimi_menu_<?php echo $menuName; ?> <?php echo in_array($menuName, $this->pullDownMiniMenus) ? ' seaocore_mimi_menu_pulldown_item' : ' ' ?>">
        ...
        <?php if( in_array($menuName, $this->pullDownMiniMenus) ): ?>
          ...
          <div class="seaocore_pulldown_wrapper pulldown_contents_wrapper <?php echo $pullDownParams['action'] == '/seaocore/mini-menu/notification' ? 'notification_pull_down_wrapper' : ''; ?> <?php echo $menuName . '_pulldown' ?>" id="<?php echo $id ?>" data-action="<?php echo $pullDownParams['action'] ?>">
...
<script type='text/javascript'>
  ...
  window.addEvent('load', function() {
    ...
    let bubbleCount = '<?php echo "(".$bubbleCount.")"; ?>';
    let count = '<?php echo $bubbleCount; ?>';

     if(count) {
       console.log('count',count);
       var text = document.createTextNode(bubbleCount);
       tag.appendChild(text); tag.classList.add("notify_count");
       var element = document.getElementsByClassName('user_popover_settings_notifications')[0];
...
<style>
  ...
  a.menu_user_popover_settings.user_popover_settings_notifications {
    display: flex;
  }
  #sitecoretheme_top_header_wrapper .layout_seaocore_menu_mini #core_menu_mini_menu .seaocore_mimi_menu_core_mini_profile .seaocore_pulldown_arrow {
  ...
  #sitecoretheme_top_header_wrapper .layout_seaocore_menu_mini #core_menu_mini_menu > ul > li .core_mini_profile_pulldown {
    margin-left: 0;
    transform: translate(-151px, 10px) !important;
  }
</style>