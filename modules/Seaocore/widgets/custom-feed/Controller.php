 ...
 */
class Seaocore_Widget_CustomFeedController extends Engine_Content_Widget_Abstract {
    ...
    public function indexAction() {
        ...
        if (Engine_Api::_()->core()->hasSubject()) {
            ...
            if (!in_array($subject->getType(), array('sitecrowdfunding_project', 'sitecrowdfundingevent_event', 'sitepage_page', 'sitepageevent_event', 'sitebusiness_business', 'sitebusinessevent_event','sitegroup_group', 'sitegroupevent_event','sitestore_store', 'sitestoreevent_event'))) {
                if (!$subject->authorization()->isAllowed($viewer, 'view')) {
                    return $this->setNoRender();
                }
            } else if (in_array($subject->getType(), array('sitecrowdfunding_project', 'sitecrowdfundingevent_event'))) {
                if ($subject->getType() == 'sitecrowdfundingevent_event'){
                    $pageSubject = Engine_Api::_()->getItem('sitecrowdfunding_project', $subject->project_id);
                }
            }
...
//    }
        ...
        if ($viewer->getIdentity() && !$this->_getParam('action_id')) {
            ...
            if (!empty($subject)) {
                ...
                }else if ($subject->getType() == 'sitecrowdfunding_project' || $subject->getType() == 'sitecrowdfundingevent_event') {
                    $pageSubject = $subject;
                    if ($subject->getType() == 'sitecrowdfundingevent_event')
                        $pageSubject = Engine_Api::_()->getItem('sitecrowdfunding_project', $subject->project_id);
                    $this->view->enableComposer = true;
