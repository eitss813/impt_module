  ...
  <div class="_form_wapper">
    <?php if( !in_array($this->pageIdentity, array('user-auth-login', 'sitelogin-auth-login')) ) : ?>
      ...
      <div class="seao_user_auth_popup _form_cont">
        ...
        <?php
          ...
          if($link == 'https://www.impactx.co/net/' || $link == 'https://stage.impactx.co/network/'){
	        $signupHref = $this->url(array('module' => 'user', 'controller' => 'signup', 'action' => 'index', 'return_url' => '64-' . base64_encode($this->url(array('action'=>'home'),"user_general")) ), 'user_signup', true);
          }else{
            $signupHref = $this->url(array('module' => 'user', 'controller' => 'signup', 'action' => 'index', 'return_url' => '64-' . base64_encode($this->url())), 'user_signup', false);
          }
        ?>
        <?php echo $_SESSION['redirectURL'];?>
        <ul class="_navigation _footer_bottom mtop10 mbot15">
          <a style="display: block;width: 100%;text-align: center;cursor: pointer;color: #888;" href="<?php echo $signupHref; ?>"><?php echo $this->translate('Don\'t have an account? Sign Up') ?> </a>
        </ul>
        <?php endif; ?>
      </div>
    <?php endif; ?>
    <?php if( !in_array($this->pageIdentity, array('user-signup-index', 'sitequicksignup-signup-index')) ) : ?>
      ...
      <div class="seao_user_signup_popup _form_cont">
        ...
        ?>  
        <?php echo $this->action('index', 'signup', $signupModule, array('disableContent' => true,   'return_url' => '64-' . base64_encode($this->url()))); ?>
        <?php if( $this->pageIdentity !== 'user-auth-login' ): ?>
          <ul class="_navigation _footer_bottom mtop10 mbot15">
            <li class="user_login_form_tab" data-role="seao_user_auth_popup">
