...
//          ->limit(100);
  ...
  public function getNewUpdatesCount(User_Model_User $user)
  {
    ...
    $select = $notificationTable->select()
      ...
      ->where("$notificationTableName.type NOT IN (?)", array('message_new', 'friend_follow_request'));
  ...
  public function getUnreadMessageCount(User_Model_User $user)
  {
    ...
    // todo: 5.2.1 Upgrade => Fixed Issue: In Rendering sign-out, notification icons in menu due to "Mysqli statement execute error : Incorrect DATETIME value: '0'"

    $rName = Engine_Api::_()->getDbtable('recipients', 'messages')->info('name');

    if($inbox_message_lastUpdated == 0){
