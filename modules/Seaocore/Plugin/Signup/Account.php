...
class Seaocore_Plugin_Signup_UserAccount extends Core_Plugin_FormSequence_Abstract
{
  ...
  public function onProcess()
  {
    ...
    $this->_registry->user = $user = Engine_Api::_()->getDbtable('users', 'user')->createRow();
      ...
      $locationParams = $data['locationParams'];
      ...
      try{
          // profile type one - regular member
          ...
          // saving the first name
          $valueRow2 = $values->createRow();
          $valueRow2->field_id = 3;
          $valueRow2->item_id = $user->getIdentity();
          $valueRow2->value = $first_name;
          $valueRow2->save();

          // saving the last name
          $valueRow3 = $values->createRow();
          $valueRow3->field_id = 4;
          $valueRow3->item_id = $user->getIdentity();
          $valueRow3->value = $last_name;
          $valueRow3->save();
      }catch (Exception $e){

      }

      $user->setDisplayName(array('first_name' => $first_name, 'last_name' => $last_name));
      $user->save();

      // GET LOCATION TABLE
      $locationTable = Engine_Api::_()->getDbtable('locations', 'user');

      $loctionV['location'] = $location;
      $loctionV['latitude'] = $latitude;
      $loctionV['longitude'] = $longitude;
      $loctionV['formatted_address'] = $formatted_address;
      $loctionV['country'] = $country;
      $loctionV['state'] = $state;
      $loctionV['zipcode'] = $zipcode;
      $loctionV['city'] = $city;
      $loctionV['address'] = $address;
      $loctionV['zoom'] = 16;
      $loctionV['user_id'] = $user->getIdentity();
...
//  }
  ...
  public function onSubmit(Zend_Controller_Request_Abstract $request)
  {
    ...
    if($stepRow->enable) {
      ...
      Engine_Api::_()->getApi('mail', 'core')->sendSystem($email, 'user_otp', array('host' => $host, 'code' => $code));
    }
    if( $this->_otpPlugin ) {
