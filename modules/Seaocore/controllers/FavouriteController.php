...
class Seaocore_FavouriteController extends Core_Controller_Action_Standard {
    ...
    public function favouriteAction() {
        ...
        if (empty($favourite_id)) {
            ...
            if (empty($favourite_id_temp[0]['favourite_id'])) {
                ...
                try {
                    ...
                    if (!empty($resource)) {
                        ...
                        // Add activity
                        if($object_type == 'sitecrowdfunding_project'){

                            $user = Engine_Api::_()->user()->getUser($loggedin_user_id);
                            $project = Engine_Api::_()->getItem('sitecrowdfunding_project', $resource_id);

                            $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($user, $project, 'project_follow');
                            if ( $action ) {
                        ...
                        if($object_type == 'sitecrowdfunding_project') {
                            ...
                            $project = Engine_Api::_()->getItem('sitecrowdfunding_project', $resource_id);
                            $owner = Engine_Api::_()->getItem('user', $getOwnerId);
                            $project_link = $view->htmlLink($host . $project->getHref(), $project->title);

                            $profile_link = $view->htmlLink($host . $user->getHref(), $user->displayname);
                            ...
                            $selectUsers = $userTable->select()
                                ...
                                ->where("$userTableName.email != ?", $user->email)
                                ->order('displayname ASC');

                            $adminMembers = $userTable->fetchAll($selectUsers);

                            foreach($adminMembers as $adminMember){
                                Engine_Api::_()->getApi('mail', 'core')->sendSystem($adminMember->email, "SITECROWDFUNDING_PROJECT_FOLLOW", array(
                                    'project_link' => $project_link,
                                    'profile_link'  =>  $profile_link,
                                    'member_name'  =>  $profile_name,
                                    'project_name' => $project->title,
                                    'owner_name' => $adminMember->displayname,
                                    'queue' => false
                                ));
                            }
                ...
                $favourite_msg_code = Zend_Registry::get('Zend_Translate')->_('FAVOURITE');
            } else {
            ...
            $favourite_msg_code = Zend_Registry::get('Zend_Translate')->_('UNFAVOURITE');
        }
        if (empty($status)) {
        ...
        $this->view->total_no_of_favourite = $num_of_contenttype;
        $this->view->favType = $favourite_msg_code;
    }
