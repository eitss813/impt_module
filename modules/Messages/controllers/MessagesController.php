...
class Messages_MessagesController extends Core_Controller_Action_User
{
  ...
  public function viewAction()
  {
    ...
    public function messageProjectAdminAction() {

        // Make form
        $this->view->form = $form = new Messages_Form_Compose();

        $form->setTitle('Send message to project admin(s)');
        ...
        if (isset($project_id)) {
            ...
            // list admins
            $listAdmins = $project->getLeaderList();
            $list_id = $listAdmins['list_id'];
            ...
            $project_admins = $userTable->fetchAll($select);

            //current user
            $viewer = Engine_Api::_()->user()->getViewer();
            $viewer_id = $viewer->getIdentity();

            // if only admin, then check if user is not logged to current user
            if(count($project_admins) === 1 ) {
                if($project_admins[0]->getIdentity() == $viewer_id){
                    // Set defaults for the form elements
                    echo '<h2 style="text-align: center;" >You cannot send message to yourself</h2>';
                    $form->removeElement('submit');
                }
            }
            $selectedRecipients = array();
            $recipients = array();
            // loop user to get its user data type
            foreach ($project_admins as $project_admin) {

                $toUser = Engine_Api::_()->getItem('user', $project_admin->getIdentity());
                $selectedRecipients[] = array(
                    'type' => 'user',
                    'id' => $project_admin->getIdentity(),
                    'guid' => $toUser->getGuid(),
                    'label' => $toUser->getTitle(),
                    'photo' => $this->view->itemPhoto($toUser, 'thumb.icon'),
                    'url' => $toUser->getHref(),
                );
            ...
            $ids = array_column($selectedRecipients, 'id');

            $idStr = implode(',', $ids);
            $form->toValues->setValue($idStr);

            // Show the users in ui
            $this->view->selectedRecipients = $selectedRecipients;
            ...
            try {
                ...
                // if logged in/current user exists remove them dont send the mail
                if(in_array($viewer_id, $recipients)) {
                    $recipients  = array_diff($recipients, array($viewer_id));
                }
    ...
    public function messageProfileUserAction() {

        // Make form
        $this->view->form = $form = new Messages_Form_Compose();
        // get project admins
        $user_id = $this->_getParam('to');
        $toUser = Engine_Api::_()->getItem('user',  $user_id);

        $form->setTitle('Send message to profile '.$toUser->getTitle());
        $form->removeDecorator('description');
  ...
  public function composeAction()
  {
    ...
    // list users
    $this->view->isPopulated = $isPopulated =  true;
