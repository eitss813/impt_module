en4.core.runonce.add(function () {
	$$('.filter-div').addEvent('click', function ($this) {
		container = this.getParent('.container');
		container.getElements('.filter-div.active').removeClass('active');
		this.addClass('active');
		showClass = '.' + this.get('data-class');
		baseClass = '.row';
		if (baseClass != showClass) {
			container.getElements(baseClass).each( function(el) { el.hide(); });
		}
		container.getElements(showClass).each( function(el) { el.show(); });
	})

	$$('.clear-logs').addEvent('click', function ($this) {
		url = en4.core.baseUrl + this.get('data-url');
		this.getParent('.container').getElements('.row').destroy();
...
function processJsonData(responseArray, container) {
	responseArray.each(function(response, index) {
		...
		row = new Element('div', {class: 'row ' + rowClasses}).inject(container);
		...
		row.adopt(new Element('div', {class: 'mtop30 pointer'})
				...
				.addEvent('click', function($this) {
					el = this.getParent('.row').getElement('.row-bottom')
					el.hasClass('hide') ? el.removeClass('hide') :  el.addClass('hide');
				}))
				)
		rowBottom = new Element('div', { class: 'row-bottom hide'}).inject(row);
		rowBottom
		.adopt(new Element('div')
		...
		.adopt(new Element('div')
			...
			.adopt(new Element('div', {class: 'inline-block fa copy width10 center'})
				.addEvent('click', function($this) {
					copyText(this.getParent().getElement('.copy-target'));
				})
				)
			)
		.adopt(new Element('div')
