...
class Siteapi_Model_DbTable_Consumers extends Engine_Db_Table {
    ...
    public function getCustomPaginator($params = array()) {
        $paginator = Zend_Paginator::factory($this->getCustomSelect($params));
        if (!empty($params['page'])) {
            $paginator->setCurrentPageNumber($params['page']);
        }
        if (!empty($params['limit'])) {
    ...
    public function getCustomSelect($params = array()) {
        $name = $this->info('name');
        ...
        if (isset($params['page_id']) && !empty($params['page_id'])) {
            $select->where($name . ".page_id =?", $params['page_id']);
        }
