...
class Socialbridge_Api_Linkedin extends Socialbridge_Api_Abstract
{
	...
	public function saveToken($params = array())
	{
		...
		if($user_id > 0)
		{
			...
			if($oldToken)
			{
				...
				// update serect token = access token (will remove)
	...
	public function getPlugin($params = array())
	{
		...
		{
			$this ->_objPlugin = new linkedin_API($api_params['key'], $api_params['secret'], $_GET['oauth_callback']);
		}
		else
		{

			$this ->_objPlugin = new linkedin_API($api_params['key'], $api_params['secret']);
		}
	}
	 ...
	 * Get contact list
	 *
	 * @return array (id, pic, name) -- not working on oAuth2.0
	 */
	public function getContacts($params = array())
	{
		if ($this -> _objPlugin)
		{
			$count = (isset($params['count'])?$params['count']:Engine_Api::_() -> getApi('settings', 'core') -> getSetting('contactimporter.contactsPerPage', 30));
            $start = (isset($params['start'])?$params['start']:0);
			$this->initConnect();
			
			$response = $this->_objPlugin->fetch("/v1/people/~/connections?start={$start}&count={$count}");
			$datatopost = null;
			$index = 0;
			foreach($connections as $con)
			{
				$datatopost .= 'id_'.$index.'@;@'.(string)$con->id.'@;@name_'.$index.'@;@'.(string)$con->{'first-name'}.' '.(string)$con->{'last-name'}.'@;@pic_'.$index.'@;@'.(string)$con->{'picture-url'}.'@;@';
				$index++;
			}
	...
	private function initConnect($params = array())
	{
	    ...
	    $this->_objPlugin->setAccessToken($access_token);
	}
	...
	public function getOwnerId($params = array())
	{
		if ($this -> _objPlugin)
		{
			$this->initConnect();
			$fields = '/v1/people/~:(id)';
			$response = $this->_objPlugin->fetch($fields);
	 ...
	 * @return array (post_id,actor_id,target_id,message,description,created_time,attachment,permalink,description_tags,type) -- not working on oAuth2.0
	 */
	public function getActivity($params = array('lastFeedTimestamp' => 0,'limit' => 5))
	{
		...
		if ($this -> _objPlugin)
		{
			...
			if($params['type'] != 'me')
			{
				...
				$response = $this->_objPlugin->fetch('/v1/people/~/network/updates'.$fields);
				if($response['success'])
		        {
	 ...
	 * Insert feeds
	 *
	 * @param array(activities, viewer, timestamp) -- not working on oAuth2.0
	 * @return bool
	 */
	 ...
	 * Send invite
	 *
	 * @param array (to string, message string, link string, uid int, access_token string) -- not working on oAuth2.0
	 * @return bool
	 */
	 ...
	 * Send invites
	 *
	 * @param array (list array, message string, link string, uid int, access_token string) -- not working on oAuth2.0
	 * @return bool
	 */
	...
	public function postActivity($params = array())
	{
		...
		if ($this -> _objPlugin)
	    {
	        try {
				...
				$params['access_token'] = $params['access_token']['access_token'];
    	        $this->initConnect($params);
	            $response = $this -> _objPlugin->share($content, FALSE);
	            return true;
	        } catch (Exception $e) {
	            return $e->getMessage();
	        }
	 ...
	 *            	 
	 * @return arr 
	 *    Array containing retrieval success, LinkedIn response.       	 -- not working on oAuth2.0
	 */
	 ...
	 * Like feed
	 *
	 * @param array -- not working on oAuth2.0
	 * @return bool
	 */
	 ...
	 *
	 * @param array
	 * @return bool -- not working on oAuth2.0
	 */
	...
	{
		return $this -> _objPlugin -> fetchAccessToken($verification_code, $redirect_uri);
	}
	public function getAuthorizationUrl($redirect_uri, $state, $scope)
	{
		return $this -> _objPlugin -> getAuthorizationUrl($redirect_uri, $state, $scope);
	}
}
