...
class Socialbridge_Api_Twitter extends Socialbridge_Api_Abstract
{
	...
	public function getContacts($params = array())
	{
		...
		try
		{
			...
			$next_cursor = -1;
			do {
				$friends = $this->_objPlugin->followersList($params['user_id'], 200, null, $next_cursor, false, true);
				...
				if (count($friends['users']) > 0) {
					foreach ($friends['users'] as $key => $users_info) {
						if ($hasInvitedUids) {
							if (in_array($users_info['id'], $params['invited_uids'])) {
								continue;
							}
						}
						$return_data[$users_info['id']] = array(
							'id' => $users_info['id'],
							'name' => $users_info['name'],
							'pic' => $users_info['profile_image_url']
						);
					}
				}
			}while($next_cursor != 0);
