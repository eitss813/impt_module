...
scriptJquery( document ).ready(function() {
  showEditor("<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesblog.review.summary', 1); ?>");
  allowReview("<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesblog.allow.review', 1); ?>");
});
...
function allowReview(value) { 
	if(value == 1) {
		...
		if(scriptJquery('#sesblog_allow_reviewother-wrapper'))
		scriptJquery('#sesblog_allow_reviewother-wrapper').show();
		...
		if(scriptJquery('#sesblog_like_comment-wrapper'))
		scriptJquery('#sesblog_like_comment-wrapper').show();
		...
		if(scriptJquery('#sesblog_allow_reviewother-wrapper'))
		scriptJquery('#sesblog_allow_reviewother-wrapper').hide();
		if(scriptJquery('#sesblog_show_pros-wrapper'))
		...
		if(scriptJquery('#sesblog_like_comment-wrapper'))
		scriptJquery('#sesblog_like_comment-wrapper').hide();
		if(scriptJquery('#sesblog_allow_share-wrapper'))
