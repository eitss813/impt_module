 ...
 * @version    $Id: AdminReviewFieldsController.php 2016-07-23 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */
 
class Sesblog_AdminReviewFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesblog_review';
  ...
  public function indexAction() {
    ...
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesblog_admin_main_review_cat', array(), 'sesblog_admin_main_review_subfields');
  ...
  public function fieldCreateAction() {
    ...
    $content_type = $this->_getParam('content_type', null);
    if ($content_type == 'sesblog_category') {
      $content_type = 'sesblog_blog';
    }
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit Event Review Category Profile Question');
    $form->removeElement('show');
    $form->removeElement('search');
    $form->addElement('hidden', 'show', array('value' => 0));
    $form->removeElement('display');
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesblog_Form_Admin_Review_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesblog_review_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesblog_review_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesblog_review_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      // Engine_Api::_()->getDbtable('categories', 'seseventreview')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
