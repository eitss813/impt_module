 ...
 * @version    $Id: _formCreateReview.tpl 2014-07-20 9:40:21Z SocialEngineAddOns $
...
<script type="text/javascript">
  ...
  function doRating(element_id, ratingparam_id, classstar) {
    $(element_id + '_' + ratingparam_id).getParent().getParent().className = 'sm-us-box rating-box ' + classstar;
    $('member_rate_' + ratingparam_id).value = $(element_id + '_' + ratingparam_id).getParent().id;
  }

  function doDefaultRating(element_id, ratingparam_id, classstar) {
    $(element_id + '_' + ratingparam_id).getParent().getParent().className = 'sm_ug_rating ' + classstar;
    $('member_rate_' + ratingparam_id).value = $(element_id + '_' + ratingparam_id).getParent().id;
  }
...
<?php if (empty($this->isajax) && $this->can_create && !$this->hasPosted): ?>
  ...
  <div class="form-wrapper" id="overall_rating">
    ...
    <div id="overall_rating-element" class="form-element">
      <ul id= 'rate_0' class='sm_ug_rating <?php echo $rating_value; ?>'>
      ...
      <input type="hidden" name='member_rate_0' id='member_rate_0' value='<?php echo $rating_value_2; ?>' />
    </div>
  </div>
      ...
      <?php foreach ($this->reviewCategory as $reviewcat): ?>
        ...
        <div class="form-wrapper">
          ...
          <div class="form-element">
            <ul id= 'rate_<?php echo $reviewcat->ratingparam_id; ?>' class='sm-us-box rating-box <?php echo $rating_value; ?>'>
            ...
            <input type="hidden" name='member_rate_<?php echo $reviewcat->ratingparam_id; ?>' id='member_rate_<?php echo $reviewcat->ratingparam_id; ?>' value='<?php echo $rating_value_1; ?>' />
          </div>
        </div>
      <?php endforeach; ?>
    <?php endif; ?>
...
<div id="thankYou" style="display: none;">
  ...
  <div class="sr_reply_thankyou_msg o_hidden">
    <?php if (empty($this->hasPosted)): ?>
      <?php if (!empty($this->viewer_id)): ?>
        <h4><i class="sr_icon_tick sr_icon mright5"></i><?php echo $this->translate("Thank You, %s!", $title); ?></h4>
      <?php else: ?>
        <h4><i class="sr_icon_tick sr_icon mright5"></i><?php echo $this->translate("Thank You!"); ?></h4>
      <?php endif; ?>
      <?php echo $this->translate("Your Review on %s has been successfully submitted.", $this->user->gettitle()); ?>
    ...
    <?php else: ?>
      ...
      <?php else: ?>
        <h4><i class="sr_icon_tick sr_icon mright5"></i><?php echo $this->translate("Thank You!"); ?></h4>
      <?php endif; ?>
      <?php echo $this->translate("Your Review on %s has been successfully updated.", $this->user->getTitle()); ?>
    <?php endif; ?>
    <ul class="mtop10 sr_reply_thankyou_msg_links clr o_hidden">
      <?php if ($this->viewer_id): ?>
        <li id="go_to_review"></li>
      <?php endif; ?>
      <li>
        <a href='<?php echo $this->user->getHref(); ?>'><?php echo $this->translate("Go back to the member profile page"); ?></a>
      </li>
    </ul>
...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    if (review_has_posted == 0) {
      showRating($('sitemember_create'), $('overall_rating'), $('rating-box'));
    } else {
      showRating($('sitemember_update'), $('overall_my_rating'), $('rating-my-edit-box'));
    }
  });
  ...
  function submitForm(review_id, formObject, type) {
    ...
    if (typeof formElement['member_rate_0'] != 'undefined' && formElement['member_rate_0'].value == 0) {
...
<?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemember.proncons', 1)): ?>
...
<?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemember.summary', 1)): ?>
...
<?php endif; ?>
    ...
    if (type == 'create') {
      url = '<?php echo $this->url(array('action' => 'create', 'user_id' => $this->user_id), "sitemember_user_general"); ?>';
    } else if (type == 'update') {
      url = '<?php echo $this->url(array('action' => 'update', 'user_id' => $this->user_id), "sitemember_user_general"); ?>';
    }
    ...
    if (flag == true) {
      ...
      var request = new Request.JSON({
        ...
        onSuccess: function(responseJSON) {
          ...
          var captcha = '<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemember.captcha', 1); ?>';
          ...
          if (viewer_id != 0 && $('go_to_review'))
            $('go_to_review').innerHTML = '<a href=' + responseJSON.review_href + '><?php echo $this->translate("View your review"); ?></a>';
          SmoothboxSEAO.close();
          SmoothboxSEAO.open($('thankYou').innerHTML);
  ...
  var withLightbox = '<?php echo $this->withLightbox; ?>';
  function showForm() {

    if ($('sitemember_create')) {
      $('sitemember_create').style.display = 'block';
      location.hash = 'sitemember_create';
      showRating($('sitemember_create'), $('overall_rating'), $('rating-box'));
    }
    if ($('sitemember_update')) {
      location.hash = 'sitemember_update';
      $('sitemember_update').style.display = 'block';
      if ($('sitemember_update').getElementById('body'))
        $('sitemember_update').getElementById('body').focus();
      showRating($('sitemember_update'), $('overall_my_rating'), $('rating-my-edit-box'));
    }

    if (withLightbox == 1) {
      $('sm_user_review_form_wapper').style.display = 'block';
      if (typeof contentReview == 'undefined') {
        contentReview = $('sm_user_review_form_wapper').innerHTML;
      }

      if (typeof contentReview == 'undefined' || contentReview) {
        SmoothboxSEAO.open('<div class="seao_smoothbox_lightbox_loading" style="display: block;"></div>');
        setTimeout(function() {
          SmoothboxSEAO.open(contentReview);
        }, 1000)
      }
      $('sm_user_review_form_wapper').innerHTML = '';
    }
    return false;
  }
