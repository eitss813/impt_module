...
class Sitemember_Api_Core extends Core_Api_Abstract {
  ...
  public function getUsersSelect($params = array(), $customParams = null) {
    ...
    $tableComplimentName = $tableCompliment->info('name');
    ...
    if(isset($params['complimentcategory_id']) && !empty($params['complimentcategory_id'])){ 
        $select->join($tableComplimentName, "$rName.user_id = $tableComplimentName.resource_id AND complimentcategory_id =".$params['complimentcategory_id'], array("$tableComplimentName.resource_id"));
    }
...
//PROCESS OPTIONS
      ...
      if($customParams){
        ...
        $checkVersion = Engine_Api::_()->seaocore()->checkVersion($coreVersion, '4.9.0');
    ...
    //Map view result if user location is not set in app.
    if (isset($params['map_view'])  && !empty($params['map_view']) && !isset($params['location'])) {
    ...
    if (isset($params['viewMembers']) && !empty($params['viewMembers'])) {
      switch ($params['viewMembers']) {
        ...
        case "top_reviewer_count":
          $select->setIntegrityCheck(false)
                  ->join($reviewsTableName, "$rName.user_id = $reviewsTableName.owner_id", array('COUNT(engine4_sitemember_reviews.review_id) AS member_count_review', 'MAX(engine4_sitemember_reviews.review_id) as max_review_id'))
                  ->group("engine4_users.user_id");
          $select->order("member_count_review DESC");
          break;
        case "top_raters":
          $select->setIntegrityCheck(false)
    ...
    if(isset($params["compliments"]) && !empty($params["compliments"])) {
        $select->where("$rName.user_id IN (?)",(array)$params["compliments"])
                ->order(new Zend_Db_Expr("FIELD($rName.user_id,".implode(",",$params["compliments"]).")"));
    }
  ...
  // alternative to array_column()
  public function getArrayByColumn($itemData, $columnName) {
      $columnData = array();
