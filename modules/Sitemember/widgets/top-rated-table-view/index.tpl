...
<?php if(empty($this->isAjax)) :?>
  <div id="dynamic_app_info_sitemember">
    <div>
      <ul class="seaocore_browse_list <?php if(!empty($this->circularImage)):?> sitemember_circular_container <?php endif;?>" id="top_rated">
     <?php endif;?> 
...
<?php  if ($this->paginator->count() > 1 && $this->paginator->count() > $this->page && empty($this->isAjax)): ?>
            <div id="pagination_container">
                <div class="seaocore_view_more" id="topratedmember_viewmore" style="display: none;">
                    <?php
                    echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array(
                        'id' => 'topratedmember_viewmore_link',
                        'class' => 'buttonlink icon_viewmore'
                    ))
                    ?>
                </div>

                <div id="topratedmember_loading" style="display: none;" class="seaocore_view_more">
                ...
                <div class="seaocore_view_more" id="topratedmember_noviewmore" style="display: none;">
                    <?php echo $this->translate('No matching results were found for members.'); ?>
                </div>
            </div>
        <?php endif; ?>
...
<script type="text/javascript">  
     ...
     en4.core.runonce.add(function() {
        <?php if ($this->paginator->count() > 1 && $this->paginator->count() > $this->page): ?>
            if ($('topratedmember_viewmore')) {
                window.onscroll = doOnScrollLoadTopRated;
                $('topratedmember_viewmore').style.display = '';
                //$('feed_viewmore').style.display = 'none';
                $('topratedmember_loading').style.display = 'none';
p
                $('topratedmember_viewmore_link').removeEvents('click').addEvent('click', function(event) {
                    event.stop();
                    getNextTopRated();
                });
            }
        ...
        <?php else: ?>
            window.onscroll = '';
            <?php if ($this->page > 1) : ?>
                $('topratedmember_noviewmore').style.display = 'block';
                $('topratedmember_loading').style.display = 'none';
                $('topratedmember_viewmore').style.display = 'none';
            <?php endif; ?>
        <?php endif; ?>
    }); 
    
    var doOnScrollLoadTopRated = function()
    {
        if ($('topratedmember_viewmore')) {
            if (typeof($('topratedmember_viewmore').offsetParent) != 'undefined') {
                var elementPostionY = $('topratedmember_viewmore').offsetTop;
            } else {
                var elementPostionY = $('topratedmember_viewmore').y;
            }
...
<script type="text/javascript">  
   ...
   function getNextTopRated() {
   
    if($('topratedmember_noviewmore')) {
        $('topratedmember_noviewmore').style.display = 'none';
      }
      if($('topratedmember_loading')) {
        $('topratedmember_loading').style.display = 'block';
      }      
      ...
      en4.core.request.send(new Request.HTML({
        ...
        data : {
         ...
         onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
                Elements.from(responseHTML).inject($('top_rated'));
