 ...
 */
class Sitemember_Widget_SearchboxSitememberController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemember.category.enabled', 1) && !empty($widgetSettings['formElements']) && in_array("categoryElement", $widgetSettings['formElements']) && !empty($widgetSettings['categoriesLevel'])) {
      $this->view->categoryElementExist = 1;
    }

    if (empty($widgetSettings['formElements']) || (!in_array("textElement", $widgetSettings['formElements']) && !Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemember.category.enabled', 1) && !in_array("categoryElement", $widgetSettings['formElements']))) {
    ...
    //PREPARE FORM
    $this->view->form = $form = new Sitemember_Form_Searchbox(array('widgetSettings' => $widgetSettings));
