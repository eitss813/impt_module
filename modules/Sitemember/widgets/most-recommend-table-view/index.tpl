...
<?php if(empty($this->isAjax)) :?>
  <div id="dynamic_app_info_sitemember">
    <div>
      <ul class="seaocore_browse_list <?php if(!empty($this->circularImage)):?> sitemember_circular_container <?php endif;?>" id="top_recommended">
          <?php endif;?>
...
<?php  if ($this->paginator->count() > 1 && $this->paginator->count() > $this->page && empty($this->isAjax)): ?>
            <div id="pagination_container">
                <div class="seaocore_view_more" id="toprecommended_viewmore" style="display: none;">
                    <?php
                    echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array(
                        'id' => 'toprecommended_viewmore_link',
                        'class' => 'buttonlink icon_viewmore'
                    ))
                    ?>
                </div>

                <div id="toprecommended_loading" style="display: none;" class="seaocore_view_more">
                ...
                <div class="seaocore_view_more" id="toprecommended_noviewmore" style="display: none;">
                    <?php echo $this->translate('No matching results were found for members.'); ?>
                </div>
            </div>
        <?php endif; ?>
...
<script type="text/javascript">  
     ...
     en4.core.runonce.add(function() {
        <?php if ($this->paginator->count() > 1 && $this->paginator->count() > $this->page): ?>
            if ($('toprecommended_viewmore')) {
                window.onscroll = doOnScrollLoadTopRaters;
                $('toprecommended_viewmore').style.display = '';
                //$('feed_viewmore').style.display = 'none';
                $('toprecommended_loading').style.display = 'none';
p
                $('toprecommended_viewmore_link').removeEvents('click').addEvent('click', function(event) {
                    event.stop();
                    getNextTopRecommended();
                });
            }
        ...
        <?php else: ?>
            window.onscroll = '';
            <?php if ($this->page > 1) : ?>
                $('toprecommended_noviewmore').style.display = 'block';
                $('toprecommended_loading').style.display = 'none';
                $('toprecommended_viewmore').style.display = 'none';
            <?php endif; ?>
        <?php endif; ?>
    }); 
    ...
    {
        if ($('toprecommended_viewmore')) {
            if (typeof($('toprecommended_viewmore').offsetParent) != 'undefined') {
                var elementPostionY = $('toprecommended_viewmore').offsetTop;
            } else {
                var elementPostionY = $('toprecommended_viewmore').y;
            }
...
<script type="text/javascript">  
   ...
   function getNextTopRecommended() {
   
            if($('toprecommended_noviewmore')) {
        $('toprecommended_noviewmore').style.display = 'none';
      }
      if($('toprecommended_loading')) {
        $('toprecommended_loading').style.display = 'block';
      }      
      ...
      en4.core.request.send(new Request.HTML({
        ...
        data : {
         ...
         onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
                Elements.from(responseHTML).inject($('top_recommended'));
