...
<ul id="profile_groups" class="groups_browse grid_wrapper">
  <?php foreach( $this->paginator as $group ): ?>
    <li>
      <div class="groups_profile_tab_photo">
        <?php echo $this->htmlLink($group, $this->itemBackgroundPhoto($group, 'thumb.profile')) ?>
      </div>
      <div class="groups_profile_tab_info">
        <div class="groups_profile_tab_title">
          <?php echo $this->htmlLink($group->getHref(), $group->getTitle()) ?>
        </div>
        <div class="groups_profile_tab_members groups_members">
          <span><i class="fa fa-user"></i></span>
          <span><?php echo $this->translate(array('%s', '%s', $group->member_count),$this->locale()->toNumber($group->member_count)) ?></span>
        </div>
