...
class User_AuthController extends Core_Controller_Action_Standard
{
    ...
    public function loginAction()
    {
        ...
        if( null === $this->_helper->contextSwitch->getCurrentContext() ) {
            ...
            if( $uri ) {
                if (substr($uri, 0, 3) == '64-') {
                    ...
                    if($uri == '/network/'){
                        ...
                        // todo: 5.2.1 Upgrade => go to returnUrl after login
            ...
            // todo: 5.2.1 Upgrade => go to activites page
    ...
    public function logoutAction()
    {
        ...
        */
        ...
        // todo: 5.2.1 Upgrade => Fix the redirect issue when logout
        // if project-id is present then go to project details
        if( !empty($project_id) && $page_id===null ){
            $project = Engine_Api::_()->getItem('sitecrowdfunding_project', $project_id);
            return $this->_forward('success', 'utility', 'core', array(
                'smoothboxClose' => true,
                'parentRedirect' => $project->getHref(),
                'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are now logged out.'))
            ));
        }
        // if page-id is present then go to organisation details
        elseif(!empty($page_id) && $project_id===null ){
            ...
            if($host == 'stage.impactx.co'){
                return $this->_forward('success', 'utility', 'core', array(
                    'smoothboxClose' => true,
                    'parentRedirect' => 'https://impactnet.co/stage',
                    'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are now logged out'))
                ));
            }else{
                return $this->_forward('success', 'utility', 'core', array(
                    'smoothboxClose' => true,
                    'parentRedirect' => 'https://www.impactx.co/',
                    'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are now logged out'))
                ));
            }
