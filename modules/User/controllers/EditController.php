...
class User_EditController extends Core_Controller_Action_User
{
    ...
    public function locationAction(){
        ...
        //GET LOCATION
        $this->view->form = $form = new User_Form_Edit_Location();
        $location = $locationTable->getLocationByUserId($viewer->getIdentity());
        if(!empty($location)){
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
            ...
            $db = $locationTable->getAdapter();
            $db->beginTransaction();
            try {

                // if location is already added, then update else create a row
                //GET FORM VALUES
                $values = $form->getValues();
                $loctionV = $values;
                $loctionV['zoom'] = 16;

                $locationTable->update($loctionV, array(
                    'user_id =?' => $user_id,
                ));
    ...
    public function editLocationAction(){
        ...
        //GET LOCATION TABLE
        $locationTable = Engine_Api::_()->getDbtable('locations', 'user');
        $location = $locationTable->getLocationByUserId($user_id);

        // Bind Form
        $this->view->form = $form = new User_Form_Edit_EditLocation();
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
            ...
            try {
                ...
                if (empty($location)) {
                    ...
                    $loctionV = Zend_Json_Decoder::decode($values['locationParams']);
            ...
            $this->view->message = Zend_Registry::get('Zend_Translate')->_('Your location saved successfully.');

            $this->_forward('success', 'utility', 'core', array(
                'smoothboxClose' => true,
                'parentRefresh' => true,
                'messages' => array(Zend_Registry::get('Zend_Translate')->_('Your location saved successfully.'))
            ));
