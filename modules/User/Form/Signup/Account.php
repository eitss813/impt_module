...
class User_Form_Signup_Account extends Engine_Form_Email
{
  public function init()
  {
    ...
    // todo: 5.2.1 Upgrade => Added signup fields which was present before upgrade
    ...
    $this->first_name->getDecorator('Description')->setOptions(array('placement' => 'APPEND', 'escape' => false));
    ...
    $this->last_name->getDecorator('Description')->setOptions(array('placement' => 'APPEND', 'escape' => false));
    ...
    $this->addElement('hidden', 'formatted_address', array('order' => 800001));
    $this->addElement('hidden', 'latitude', array('order' => 900001));
    $this->addElement('hidden', 'longitude', array('order' => 900002));
    $this->addElement('hidden', 'country', array('order' => 900003));
    $this->addElement('hidden', 'state', array('order' => 900004));
    $this->addElement('hidden', 'city', array('order' => 900005));
    $this->addElement('hidden', 'zipcode', array('order' => 900006));
    $this->addElement('hidden', 'address', array('order' => 900007));


    // todo: 5.2.1 Upgrade => Added redirect code logic
    // set the redirect form url
    $zendRequest = Zend_Controller_Front::getInstance()->getRequest();
    $returnedUrl = $zendRequest->getParam('return_url', null);

    if($_SERVER[REQUEST_URI] != '/signup'){
        $custom_redirect_url =  (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";

        // if returned url is set in url, then redirect to that instead of custom
        if(!empty($returnedUrl)){
            $custom_redirect_url = null;
        }

        $this->addElement('hidden', 'custom_redirect_url', array(
            'order' => 900008,
            'value' => $custom_redirect_url
        ));
        $this->addElement('hidden', 'given_redirect_url', array(
            'order' => 900009,
            'value' => $returnedUrl
        ));
