...
?>
<?php if(empty($this->page_layout) && empty($this->initiative_id) && empty($this->page_layout)):?>
<h3>Sustainable Development Goals</h3>
<?php endif; ?>

<div class="chart_container" id="chart_containerchart_container">
    <div class="component">
        <div class="smartcms_wrapper opened-nav" id="smartcms_wrapper">
            <ul>
                ...
                ?>
                <?php $goalId =  $goal['Project_Primary_key']; $projectTotalCount = $goal['total_projects_count']; ?>
    ...
    <?php foreach($this->goals as $goal): ?>
      <?php if($goal['total_projects_count'] > 0) {
        $sdgCount = $sdgCount + 1;
                foreach($goal['projects'] as $val):
    ...
    <div class="sitecoretheme_icons_wrapper _cards _round_icons" id="inner_content">
        <div class="sitecoretheme_icons_inner" >
            <div class="sitecoretheme_icons_content_4 wow  fadeInUp animated" style="visibility: visible; animation-name: fadeInUp;"  id="hide_card2">
                        ...
                        <div class="_image_icon">
                          <span>
                            <p>
                                ...
                                <?php
                                   $allPartnerPages = Engine_Api::_()->getDbtable('pages', 'sitepage')->getPageDetailsWithProjectsCustomCountForPageIds($this->page_id);
                                   ...
                                   $routeName = Zend_Controller_Front::getInstance()->getRouter()->getCurrentRouteName();
                                    echo  ('sitepage_initiatives' == $routeName) ? $this->initiative_project_count : $projectsCount;

                                ?>
                            </p>
                          </span>
                        </div>
                        <h4 class="services_desc">PROJECT(S)</h4>
                    </div>
            ...
            <div class="sitecoretheme_icons_content_4 wow  fadeInUp animated" id="project_details"  style="display: none">
                <div class="sitecoretheme_icons_content_4_inner" id="project_card_details" >
                    <div class="chart_content_container" >
                        <?php if(count($goal['total_projects_count']) > 0): ?>
                        <?php foreach($this->goals as $goal): ?>

                        <div id="information_id_<?php echo $goal['Project_Primary_key'] ?>" class="hide_this common_information">

                            <div class="_image_icon" style="cursor: pointer"  onclick='goToSearchPage("<?php echo $goal['Project_Primary_key']?>","<?php echo $goal['target_ids_str'] ?>");'>
                                <span>
                                 <p>   <?php echo $goal['total_projects_count'];?></p>
                                </span>
                            </div>
                            <a class="goals_projects_count" href="javascript:void(0);" onclick='goToSearchPage("<?php echo $goal['Project_Primary_key']?>","<?php echo $goal['target_ids_str'] ?>");'>
                            PROJECT(S)
                            </a>
                        ...
                        <div class="tip custom-tip">
                            <span>
                                <?php if(empty($this->page_layout) && empty($this->initiative_id) && empty($this->page_layout)):?>
                                <?php echo $this->translate('There is no goals attached to any projects in this organisation'); ?>
                                <?php else:?>
                                <?php echo $this->translate('There is no goals attached to any projects in this initiatives'); ?>
                                <?php endif; ?>
                            </span>
                        </div>
                        <?php endif; ?>
...
<script type="text/javascript">
    ...
    function goToSearchPage(goalId,goalTargetId){
        document.getElementsByName('query')[0].value=null;
        $j('#query').val(null);
        $j('#sdg_goal_id').val(goalId);
        $j('#sdg_target_id').val(goalTargetId);
        $j('#search_only_in_project').val(true);
        $j('#category_id').val(null);
        var page_id = "<?php echo $this->page_id;?>";
        $j('#searched_from_page_id').val(page_id);
        var initiative_id  = "<?php echo $this->initiative_id;?>";
        $j('#searched_from_initiative_id').val(initiative_id);
        document.getElementById("organisation_search_form").submit();
    }
