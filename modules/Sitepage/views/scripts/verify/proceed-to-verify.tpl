...
<?php if ($this->verify_count > 0): ?>
  <div id="<?php echo 'user'; ?>_successMessege_<?php echo $this->resource_id; ?>" style ='<?php echo $successMessege_show; ?>'>
    <div class="seaocore_txt_light o_hidden">
      ...
      <?php endif; ?>
      <span class="o_hidden sitepage_verify_label"><?php echo $this->translate("%s has been verified by", ucfirst($this->resource_title)) . ' ' . $this->translate(array('%s member', '%s members', $this->verify_count), $this->locale()->toNumber($this->verify_count)) . '.'; ?></span>
    </div>
    <a class="sitepage_links mbot10 f_small" href="javascript:void(0)" onclick="Smoothbox.open('<?php echo $this->url(array('module' => 'sitepage', 'controller' => 'verify', 'action' => 'content-verify-member-list', 'resource_id' => $this->resource_id,), 'default', true) ?>');"><?php echo $this->translate("View Details") . " &raquo;"; ?></a>
    <div id="<?php echo 'user'; ?>_successMessege_<?php echo $this->resource_id; ?>" class="o_hidden clr"  style ='<?php echo $sitepage_ownerprofile; ?>'>
      <div><?php echo $this->translate('You have verified this page.'); ?> </div>
      <div class="sitepage_links f_small">
