...
class Payment_Plugin_Gateway_2Checkout extends Engine_Payment_Plugin_Abstract
{
  ...
  public function cancelSubscription($transactionId)
  {
    ...
    try {
      $salesInfo = $this->getService()->detailSale($profileId);
    } catch (Exception $ex) {
      $salesInfo = $this->getService()->detailInvoice($profileId);
    }

    if (!isset($salesInfo['invoices'])) {
      return $this;
    }

    // Stop each recurring line item
    foreach($salesInfo['invoices'] as $invoiceData) {
      foreach ($invoiceData['lineitems'] as $lineitem) {
        $lineitemIds[] = $lineitem['lineitem_id'];
      }
    }

    $lineitemIds = array_unique($lineitemIds);
    foreach ($lineitemIds as $lineitemId) {
      try {
        $r = $this->getService()->stopLineItemRecurring($lineitemId);
      } catch (Exception $ex) {
        // Listen silently
      }
    }
