...
class Core_AdminIndexController extends Core_Controller_Action_Admin
{
    ...
    {
        $this->_helper->layout->setLayout('admin-ajax');
        $warehouse = new Engine_Warehouse('/products');
        try {
            $response = $warehouse->setCache(1800)->get(array(
                'certified' => '1'
            ));
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            try {
                ...
                if ($this->getParam('popup')) {
                    ...
                    $params['messages'] = array(Zend_Registry::get('Zend_Translate')->_("License saved"));
                    $this->_forward('success', 'utility', 'core', $params);
                } else {
                    $this->view->keyUpdated = true;
                }
            } catch (Exception $e) {
                $db->rollback();
