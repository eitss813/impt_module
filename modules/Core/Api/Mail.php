        ...
        foreach( $recipients as $recipient ) {
            ...
            if(!is_string($recipient) ) {
                ...
                if($_SERVER['SERVER_NAME'] == 'stage.impactx.co'){
                    $editEmailPreferencesUrl = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/network/members/settings/emails';
                }else{
                    $editEmailPreferencesUrl = (_ENGINE_SSL ? 'https://' : 'http://') . $_SERVER['HTTP_HOST'] . '/net/members/settings/emails';
                }
            ...
            $emailSettingsDetails = null;
            $user = Engine_Api::_()->getItemTable('user')->fetchRow(array('email LIKE ?' => $recipientEmail));
            if ($user) {
            ...
            //if this match found the it means we should not send mail to that $recipientEmail
            if (in_array($type, $emailSettingsDetails)) {
                //do need to send mail
            }
            else {
                //send email
                $this->sendRaw($mail);
