    ...
    <?php  $this->partial()->setObjectKey('sitepagevideo');
		...
		?>		       
          ...
          $sitepage  = Engine_Api::_()->getItem('sitepage_page', $sitepagevideo->page_id);
					...
					$tmpBody = strip_tags($sitepage->title);
					$page_title = ( Engine_String::strlen($tmpBody) > $truncation_limit ? Engine_String::substr($tmpBody, 0, $truncation_limit) . '..' : $tmpBody );
          ?>
          <?php echo $this->translate("in ") . $this->htmlLink(Engine_Api::_()->sitepage()->getHref($sitepagevideo->page_id, $sitepagevideo->owner_id, $sitepagevideo->getSlug()), $page_title, array('title' => $sitepage->title)) ?>    
          </div>
          <div class="sitepage_sidebar_list_details"> 
					<?php echo $this->translate(array('%s like', '%s likes', $sitepagevideo->like_count), $this->locale()->toNumber($sitepagevideo->like_count)) ?>,
