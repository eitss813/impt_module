...
<script type="text/javascript">
    ...
    // set markers
    function setMarkers(){
        <?php  if (count($this->paginatorMapView) > 0) : ?>
            <?php foreach ($this->paginatorMapView as $item) : ?>

                <?php $location = Engine_Api::_()->getDbtable('locations', 'sitecrowdfunding')->getLocation(array('id' => $item->project_id)); ?>;

                var projectlat = <?php echo (empty($location->latitude) ? 0 : $location->latitude) ?>;
                var projectlng = <?php echo (empty($location->longitude) ? 0 :$location->longitude) ?>;
