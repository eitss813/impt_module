...
<div class="sitecrowdfunding_dashboard_content">
    ...
    <?php if($this->layoutType !='projectStepsCreate'): ?>
        <?php echo $this->partial('application/modules/Sitecrowdfunding/views/scripts/dashboard/header.tpl', array('project' => $this->project, 'sectionTitle'=>'Add New Goal', 'sectionDescription'=> 'Add New Goal using below form for this project.')); ?>
    <?php endif; ?>
...
<script>
    ...
    $j(document).ready(function() {
        ...
        $j('#sdg_goal_id').selectric({
            onChange: function(element) {
                ...
                updateTargetSelect(selectedval)
            },
        });
        updateTargetSelect(1)
        /*
        this is for normal select
        $j("#sdg_target_id").children('option').hide();
        $j('#sdg_goal_id').change(function(){
            updateTargetSelect($j(this).val())
        })
        updateTargetSelect(1)
        */
    })

    function updateTargetSelect(id){
    ...
    // this is for normal select
    // showing hiding target based on select goal
    /*function updateTargetSelect(id){
        $j("#sdg_target_id").children('option').hide();
        if (id){
            let setSelectedValue = false
            $j('#sdg_target_id > option').each(function() {
                let thisval = $j(this).val()
                thisval = thisval.split('-')
                console.log(thisval[0])
                if(thisval[0] == id){
                    $j(this).show()
                    if(!setSelectedValue){
                        setSelectedValue = true
                        $j('#sdg_target_id').val($j(this).val())
                    }
                }else{
                    $j(this).hide()
                }
