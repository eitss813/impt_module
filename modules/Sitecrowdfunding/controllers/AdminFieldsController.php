 ...
 */
class Sitecrowdfunding_AdminFieldsController extends Fields_Controller_AdminAbstract {
...
//        Make navigation
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sitecrowdfunding_admin_main', array(), 'sitecrowdfunding_admin_main_fields');
    ...
    public function fieldEditAction() {
        ...
        //START CUSTOMIZATION BY SOCIALENGINEADDONS
        $form->setTitle('Edit Project Question');
        $form->removeElement('show');
        $form->addElement('hidden', 'show', array('value' => 0));
    ...
    public function headingCreateAction() {
        ...
        if ($form) {
            $form->removeElement('show');
            $form->addElement('hidden', 'show', array('value' => 0, 'order' => '1000'));

            $form->removeElement('display');
            $form->addElement('hidden', 'display', array('value' => 1, 'order' => '1001'));
        }
    }
    ...
    public function typeCreateAction() {
        ...
        // Create form
        $this->view->form = $form = new Sitecrowdfunding_Form_Admin_Type();
        ...
        if ($form->getValue('duplicate') != 'null') {
            // Create New Option in engine4_sitecrowdfunding_project_fields_options
            ...
            $field_map_array = $db->select()
                    ->from('engine4_sitecrowdfunding_project_fields_maps')
            ...
            $field_meta_array = $db->select()
                    ->from('engine4_sitecrowdfunding_project_fields_meta')
            ...
            // Copy each row
            for ($c = 0; $c < $field_map_array_count; $c++) {
                $db->insert('engine4_sitecrowdfunding_project_fields_meta', array(
            ...
            for ($i = 0; $i < $map_count; $i++) {
                $db->insert('engine4_sitecrowdfunding_project_fields_maps', array(
    ...
    public function typeDeleteAction() {
        ...
        if (!empty($option_id)) {
            ...
            Engine_Api::_()->getDbtable('projects', 'sitecrowdfunding')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));

            //DELETE MAPPING
            Engine_Api::_()->getDbtable('categories', 'sitecrowdfunding')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
        }
