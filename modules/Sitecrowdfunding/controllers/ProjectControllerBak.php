 ...
 */
class Sitecrowdfunding_ProjectwwwwwwController extends Seaocore_Controller_Action_Standard {
    ...
    public function createAction() {
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            $values['category_id'] = $values['category_id2'];
            unset($values['category_id2']);
            ...
            if ($days > $daysCompare) {
                $error = $this->view->translate('Please do not enter duration more than 1825 days(5 Years).');
                if ($daysCompare == 90) {
                    $error = $this->view->translate('Please do not enter duration more than 90 days.');
                }
                $error = Zend_Registry::get('Zend_Translate')->_($error);
                $form->getDecorator('errors')->setOption('escape', false);
                $form->addError($error);
...
//                        'sponsored' => $package->sponsored,
                ...
                if ($days < $daysCompare && $values['is_fund_raisable'] === 1) {
                ...
                $projectPagesTable = Engine_Api::_()->getDbtable('pages', 'sitecrowdfunding');
                $organizations_ids=$values['organization_ids'];
                foreach($organizations_ids as $item){
                    $pagerow = $projectPagesTable->createRow();
                    $pagerow->project_id =$project_id;
                    $pagerow->page_id = $item;
    ...
    public function editAction() {
        ...
        if($project->approved && $project->state== 'published' && $project->is_fund_raisable){
            //once appoved and published status we can not edit status
            //if needed we can disapprove in admin side
            $form->removeElement('state');
        }
        if (!$viewerIsAdmin) {
//            $form->starttime->setAttrib('disabled', 'disabled');
//            $form->endtime->setAttrib('disabled', 'disabled');
//            $form->goal_amount->setAttrib('disabled', 'disabled');
//            $form->is_fund_raisable->setAttrib('disabled', 'disabled');
//            $form->is_fund_raisable->setRequired(false);
//            if ($form->lifetime) {
//                $form->lifetime->setAttrib('disabled', 'disabled');
//                $form->lifetime->setAllowEmpty(true);
//                $form->lifetime->setRequired(false);
//            }
...
//            $form->goal_amount->setRequired(false);
        ...
        $form->getElement('category_id2')->setValue($project->category_id);

        //populating organization ids
        $sitePagesTable = Engine_Api::_()->getDbtable('pages', 'sitecrowdfunding');
        $organizations_ids=$sitePagesTable->getProjectPages($project_id);
        if(!empty($organizations_ids)){
            $form->getElement('organization_ids')->setValue($organizations_ids);
        }
        //populating organization ids
        ...
        //goal amount is not populating with $populatedArray so added below line
        $form->populate(array( 'goal_amount' => $project->goal_amount));
        ...
        if (!$this->getRequest()->isPost()) {
            ...
            if($project->start_date !== null && $project->expiration_date !== null){
                $projectdateinfo['starttime'] = $project->start_date;
                $projectdateinfo['endtime'] = $project->expiration_date;
                $days = Engine_Api::_()->sitecrowdfunding()->findDays($project->expiration_date, $project->start_date);
                $form->populate(array(
                    'starttime' => $projectdateinfo['starttime'],
                    'endtime' => $projectdateinfo['endtime'],
                ));
            }
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            //!empty($project->backer_count) &&
            if ( !$viewerIsAdmin) {
                if ($project->start_date != $values['starttime'] || $project->expiration_date != $values['endtime']) {
                    $error = $this->view->translate('This project has been backed - You can not change Start Date and End Date');
                    $error = Zend_Registry::get('Zend_Translate')->_($error);
                    $form->getDecorator('errors')->setOption('escape', false);
                    $form->addError($error);
                    return;
            ...
            // if project previously not fund raisable and then after sometime if they changing into raising fund.
            // we need to dis-approve project so admin will again apporve this
            if($project->is_fund_raisable == 0 && $values['is_fund_raisable'] == 1){
                $values['approved'] = 0;
                $values['status'] = 'initial';
            }
            $table = Engine_Api::_()->getItemTable('sitecrowdfunding_project');
            $db = $table->getAdapter();
            $db->beginTransaction();
            ...
            try {
                ...
                //delete previous linked record
                $projectPagesTable = Engine_Api::_()->getDbtable('pages', 'sitecrowdfunding');
                $projectPagesTable->deleteLinkedRecord($project_id);
                //delete previous linked record

                //inserting new linked records
                $organizations_ids=$values['organization_ids'];
                foreach($organizations_ids as $item){
                    $pagerow = $projectPagesTable->createRow();
                    $pagerow->project_id =$project_id;
            ...
            return $this->_helper->redirector->gotoRoute(array('action' => 'edit', 'project_id' => $project->project_id), 'sitecrowdfunding_specific', true);
        }
    }
        ...
        if ($project->parent_type && $project->parent_id) {
            ...
            if (!Engine_Api::_()->sitecrowdfunding()->canDeletePrivacy(null, null, $project))
                return $this->_forward('success', 'utility', 'core', array(
                            'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'sitecrowdfunding_project_general', true),
