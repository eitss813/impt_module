...
function createMarker(latlng,html,title_page) {
        ...
        <?php  if (count($this->locations) > 0) : ?>
            ...
            <?php foreach ($this->locations as $location) : ?>
                ...
                if(markerIndex !== -1){
                    ...
                    // as we have ajax concept, it calls function 2times, so check if project is added already
                    var projectIndex = finalMarkers[markerIndex].projects.findIndex(p => p.project_id === project_id);
                    if(projectIndex === -1){
