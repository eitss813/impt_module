    ...
    <?php foreach ($this->paginator as $item): ?>
        <li>
            <div class="sitecrowdfunding_thumb_wrapper sitecrowdfunding_thumb_viewer"  style=" width:<?php echo $this->projectWidth; ?>px; height:<?php echo $this->projectHeight; ?>px;">
                ...
                <?php 
                if ($item->photo_id) {  
                    echo $this->htmlLink($item->getHref(), "<span class='project_overlay'></span>" . $fsContent . $this->itemBackgroundPhoto($item, $this->thumbnailType, null, array('tag' => 'i')));
                } else {
                  $url = $this->layout()->staticBaseUrl . "application/modules/Sitecrowdfunding/externals/images/nophoto_project_thumb_profile.png";
                     echo $this->htmlLink($item->getHref(), "<span class='project_overlay'></span>" . $fsContent . "<i style='background-image:url(" . $url . ")'></i>");
                }
                ?>
                <div class='sitecrowdfunding_hover_info'>
                  <div class="sitecrowdfunding_desc sitecrowdfunding_stats sitecrowdfunding_grid_stats">
                    <?php echo $this->sitecrowdfundingShareLinks($item, $this->projectInfo); ?>
                  </div> 
                  <?php $content=''; ?>
                  <?php $content .= "<div class='txt_center sitecrowdfunding_likes_comment_wrapper'>"; ?>
...
<div class="txt_right">
<?php echo $this->htmlLink(array('route' => "sitecrowdfunding_project_general", 'action' => 'browse'), $this->translate('Explore All'), array('class' => 'common_btn view_btn'))?>
</div>
