 ...
 * @version    $Id: Photo.php 2017-03-27 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitecrowdfunding_Model_Photo extends Core_Model_Item_Collectible {

    protected $_parent_type = 'sitecrowdfunding_album';
    protected $_owner_type = 'user';
    protected $_searchColumns = array("");
    protected $_collection_type = 'sitecrowdfunding_album';
    ...
    public function getHref($params = array()) {
        ...
        $params = array_merge(array(
            'route' => "sitecrowdfunding_image_specific",
            'reset' => true,
    ...
    protected function _postDelete() {
        ...
        try {
            ...
            if (($album instanceof Core_Model_Item_Collection) && ($nextPhoto instanceof Core_Model_Item_Collectible) &&
                ...
                $album->save();
                $subject_sitecrowdfunding = Engine_Api::_()->getItem('sitecrowdfunding_project', $this->project_id);
                $subject_sitecrowdfunding->photo_id = $nextPhoto->file_id;
                $subject_sitecrowdfunding->save();
                $subject_sitecrowdfunding->updateAllCoverPhotos();
            } elseif (($album instanceof Core_Model_Item_Collection) && (int) $album->photo_id == (int) $this->file_id) {
                Engine_Api::_()->getItemTable('sitecrowdfunding_project')->update(array('photo_id' => 0), array('photo_id = ?' => $this->file_id, 'project_id = ? ' => $this->project_id));
                Engine_Api::_()->getItemTable('sitecrowdfunding_album')->update(array('photo_id' => 0), array('photo_id = ? ' => $this->file_id, 'album_id = ? ' => $this->album_id));
            }
        } catch (Exception $e) {
