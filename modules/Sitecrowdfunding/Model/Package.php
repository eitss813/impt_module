 ...
 */
class Sitecrowdfunding_Model_Package extends Core_Model_Item_Abstract {
    ...
    public function hasProjects() {
        $table = new Sitecrowdfunding_Model_DbTable_Projects();
        $count = $table->select()->from($table->info('name'),'count(package_id)')->where('package_id = ?',$this->package_id)->query()->fetchColumn();
        if($count == 0)
            return false;
        return true;
    }
    ...
    public function getProduct() {
        if (null === $this->_product) {
            ...
            $this->_product = $productsTable->fetchRow($productsTable->select()
                            ->where('extension_type = ?', 'sitecrowdfunding_package')
    ...
    public function getProductParams() {
        return array(
            ...
            'price' => $this->price,
            'extension_type' => 'sitecrowdfunding_package',
    ...
    public function getPackageDescription() {
        ...
        $priceStr = Engine_Api::_()->sitecrowdfunding()->getPriceWithCurrency($this->price);
