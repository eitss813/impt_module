...
class Activity_View_Helper_GetActionContent extends Zend_View_Helper_Abstract
{
    ...
    public function getActionContent(Activity_Model_Action $action, $similarActivities = array(),$showUserName,$showContent )
    {
        ...
        if( empty($similarActivities) || !isset($similarActivities[$similarFeedType]) ) {
            ...
            // get both content and username
            if($showUserName === true && $showContent === true){
                return $action->getContent();
            }
            // get only content
            elseif($showUserName === false && $showContent === true){
                return $action->getContentOnly();
            }
            // get only user name
            elseif($showUserName === true && $showContent === false){
                return $action->getContentUserName();
            }
            // get both content and username
            else{
                return $action->getContent();
        ...
        // get only content
        elseif($showUserName === false && $showContent === true){
            return $action->getContentOnly($otherItems);
        }
        // get only user name
        elseif($showUserName === true && $showContent === false){
            return $action->getContentUserName($otherItems);
        }
        // get both content and username
        else{
            return $action->getContent($otherItems);
