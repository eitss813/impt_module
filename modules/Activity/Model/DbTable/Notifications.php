...
class Activity_Model_DbTable_Notifications extends Engine_Db_Table
{
                                    ...
                                    Core_Model_Item_Abstract $object, $type, array $params = null)
    {
        ...
        if ($notificationSettingsTable->checkEnabledNotification($user, $type) && !empty($user->email)) {
            ...
            if(($type =='commented' && $params['paramtype']='commented') ||
                ($type =='commented_commented' && $params['paramtype']='commented_commented') ||
                ($type =='liked_commented' && $params['paramtype']='liked_commented')) {
            ...
            // Send
            try {
                if($object->getType() == 'sitecrowdfunding_project'){
                    ...
                    foreach($adminMembers as $adminMember){
                        Engine_Api::_()->getApi('mail', 'core')->sendSystem($adminMember,
    ...
    //add notification with action id
    public function addNotificationWithActionId(User_Model_User $user, Core_Model_Item_Abstract $subject,
                                                Core_Model_Item_Abstract $object, $type, array $params = null,$link)
    {
        // Donot send notification if user is blocked
        if ($subject instanceof User_Model_User && ($user->isBlocked($subject) || $subject->isBlocked($user))) {
            return;
        ...
        if ($notificationSettingsTable->checkEnabledNotification($user, $type) && !empty($user->email)) {
            ...
            try {
                ...
                if ($objectParent && !$objectParent->isSelf($object)) {
                    $defaultParams['object_parent_title'] = $objectParent->getTitle();
                    $defaultParams['object_parent_link'] = $link;
                    $defaultParams['object_parent_photo'] = $objectParent->getPhotoUrl('thumb.icon');
                    $defaultParams['object_parent_description'] = $objectParent->getDescription();
                }
            } catch (Exception $e) {
            ...
            try {
                ...
                if ($objectOwner && !$objectOwner->isSelf($object)) {
                    $defaultParams['object_owner_title'] = $objectOwner->getTitle();
                    $defaultParams['object_owner_link'] = $link;
                    $defaultParams['object_owner_photo'] = $objectOwner->getPhotoUrl('thumb.icon');
                    $defaultParams['object_owner_description'] = $objectOwner->getDescription();
                }
            } catch (Exception $e) {
...
//            ->where("`object_type`='messages_conversation' and `object_id` NOT IN (?)", $disabledMessages);
            ...
            ->where('user_id = ?', $user->getIdentity())
            ->where('`type` IN(?)', $enabledNotificationTypes)
            ->where(" `object_id` NOT IN (?)", $disabledMessages);
