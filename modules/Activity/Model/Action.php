...
class Activity_Model_Action extends Core_Model_Item_Abstract
{
    ...
    // 12th june 2020
    // Added: Custom function to get user name or content or both

    // get only content
    public function getContentOnly($others = array()) {
        $model = Engine_Api::_()->getApi('core', 'activity');
        $params = array_merge(
            $this->toArray(),
            (array) $this->params,
        ...
        // Remove subject word from string
        $body = str_replace('{item:$subject:$label} is', '', $body);
        $body = str_replace('{actors:$subject:$object}:', '', $body);
        $body = str_replace('{item:$subject} -', '', $body);
        $body = str_replace('{item:$subject}’s project', '', $body);
        $body = str_replace('{item:$subject}’s', '', $body);
        $body = str_replace('{item:$subject} is', '', $body);
        $body = str_replace('{item:$subject}', '', $body);
        $body = trim($body);
        $body = ucfirst($body);
    ...
    // get only user name
    public function getContentUserName($others = array()) {
        $model = Engine_Api::_()->getApi('core', 'activity');
        $params = array_merge(
            $this->toArray(),
            (array) $this->params,
