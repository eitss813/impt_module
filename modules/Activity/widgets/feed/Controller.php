...
class Activity_Widget_FeedController extends Engine_Content_Widget_Abstract
{
    ...
    public function indexAction()
    {
        ...
        $actionTable = Engine_Api::_()->getDbtable('actions', 'activity');
         ...
         $this->view->actualLink = $actualLink;
        // Get some options
        ...
        //project based user id
        if($project_id ) {
            $project = Engine_Api::_()->getItem('sitecrowdfunding_project', $project_id);


            if($project->notify_project_comment || $project->notify_project_comment==0 || $project->notify_project_comment=='' || $project->notify_project_comment==null) {
                $level = $project->notify_project_comment;
                $levelArr = explode(" ",$level);
                $idsArr = explode(",",$levelArr[0]);
                if(!$project->notify_project_comment) {
                    array_push($idsArr,0);
                }
                ...
                foreach ($idsArr as $level_id) {
                    ...
                    //Members
                    if($level_id == 1) {
                        //get team members of the project start
                        ...
                        //get team members of the project end

                    }
                    //Followers
                    if($level_id == 2) {
                        // get followers of the  project start
                        ...
                        // get followers of the  project end
                    }
                    //Admins or Owner
                    if($level_id == 3 || $level_id == 0) {
                        ...
                        if(!empty($project)) {
                            ...
                            foreach ($adminMembers as $id) {
                                ...
                                if($level_id == 0) {     // Owner
                                    if((int)$project->owner_id == (int)$id['user_id']) {
                                        array_push($user_ids,$id['user_id']);
                                    }
                                }
                                if($level_id == 3) {     //Admins
                            ...
                            //get admin members of the project end
        ...
        $this->view->user_ids = $user_ids ? $user_ids:null;
    }
