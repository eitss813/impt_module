...
//                    $user_idsArr = explode(",",$user_ids);
//
//                    //add notification
//                    foreach ($user_idsArr as $user_id) {
//                        $user_subject = Engine_Api::_()->user()->getUser($user_id);
//                        if($subject->getType() == 'sitecrowdfunding_project') {
//                            $notificationType = 'commented';
//                            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($user_subject, $viewer, $subject, $notificationType, array(
//                                'url1' => $subject->getHref(),
//                            ));
//                        }
//                    }
            ...
            // if feed if  project
            if($action->object_type == 'sitecrowdfunding_project'){
            ...
            // if feed is organisation
            if($action->object_type == 'sitepage_page'){
                $sitepage = Engine_Api::_()->getItem('sitepage_page', $action->object_id);
                $sitepage->comment_count = $sitepage->comment_count + 1;
                $sitepage->save();
            }
        ...
        if (null === $this->_helper->contextSwitch->getCurrentContext()) {
            ...
            // Hide the token concept for feed flow
    ...
    {
        $user_ids =  $this->_getParam('user_id');
        // $subject =    $this->_getParam('subject');
        // Process
        //$table = Engine_Api::_()->getDbtable('actions', 'activity')->getAdapter();
        $table =  Engine_Api::_()->getDbtable('actions', 'activity');
        $select = $table->select()
            ->order('action_id DESC')->limit(1);
        $row = $table->fetchRow($select);
        $lastId = $row->action_id;
        ...
        $cpmmentSelect = $commentTable->select()
            ->order('comment_id DESC')->limit(1);
        $row = $table->fetchRow($cpmmentSelect);
        $commentLastId = $row->comment_id;

        //add notification
        foreach ($user_idsArr as $user_id) {
            $user_subject = Engine_Api::_()->user()->getUser($user_id);
            $link =  $subject->getHref().'/'.'action_id/'.$action_id;

            if($subject->getType() == 'sitecrowdfunding_project') {
                $notificationType = 'commented';
                Engine_Api::_()->getDbtable('notifications', 'activity')->addNotificationWithActionId($user_subject, $viewer, $subject, $notificationType, array(
    ...
    public function commentAction()
    {
        ...
        try {
            ...
            $cpmmentSelect = $commentTable->select()
                ->order('comment_id DESC')->limit(1);
            $row = $commentTable->fetchRow($cpmmentSelect);
            $commentLastId = $row->comment_id;
            $link =  $action->getHref().'/comment-'.$commentLastId;
        ...
        if (null === $this->_getParam('format', null)) {
            ...
            $this->view->body = $this->view->activity($action, array('noList' => true, 'action_id' => $action_id), $method, $show_all_comments);
        }
    }
    ...
    public function shareAction()
    {
        ...
        $form->body->setValue(null);
        if ($action) {
//            echo '<div id="success_toast" style="display:flex;text-align: center;text-align: center;position: absolute; top: 6px;left: 33%;border-radius: 5px;padding: 4px 16px; font-size: 14px;background-color: #44AEC1;color: #ffffff;border: 2px solid #44AEC1;outline: none;
//            " ><p>Shared Succesfully</p> <p style="cursor: pointer;font-weight: 600;" onclick="closeTab()"> &nbsp;&nbsp;X</p></div>';
...
//             </script>";
            ...
            return $this->_forward('success', 'utility', 'core', array(
                ...
                'smoothboxClose' => 3000,
                'format' => 'smoothbox',
                'messages' => array('Shared Succesfully.'),
            ));
        ...
        echo"<script language='javascript'>
                       function closeTab() {   
                           console.log('check',document.getElementById('success_toast'));
                         document.getElementById('success_toast').style.display ='none';
                       }
                       function goBack() {
                           window.history.back();
                       }
    ...
    public function deleteAction()
    {
        ...
        // Decrease the comment count if comment is delete
        // if feed if  project
        if($action->object_type == 'sitecrowdfunding_project'){
            $project = Engine_Api::_()->getItem('sitecrowdfunding_project', $action->object_id);
    ...
    //like customised code
    public function likemodalAction() {
        $action_id = $this->_getParam('action_id',null);
        $comment_id = $this->_getParam('comment_id',null);
        //checking via comment file or text file
        if($comment_id && $action_id) {
            $action  = Engine_Api::_()->getDbtable('actions', 'activity')->getActionById($action_id);
            $comment = $action->comments()->getComment($comment_id);
            $users_liked = $comment->likes()->getAllLikesUsers();

        }
        else {
            $action  = Engine_Api::_()->getDbtable('actions', 'activity')->getActionById($action_id);
            $users_liked = $action->likes()->getAllLikesUsers();
        }

        $this->view->users_liked = $users_liked;
