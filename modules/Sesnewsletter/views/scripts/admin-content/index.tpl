...
<?php $allowedAdminTitle = array('core.html-block', 'core.rich-text-block'); ?>
...
<script type="text/javascript">
  ...
  var currentLayout = '<?php echo $this->templateObject->layout ?>';
  ...
  var loadPage = function(template_id) {
    if( confirmPageChangeLoss() ) {
      window.location.search = '?page=' + template_id;
  ...
  var saveChanges = function()
  {
    ...
    var url = '<?php echo $this->url(array('action' => 'update', 'controller' => 'content', 'module' => 'sesnewsletter'), 'admin_default', true)?>';
  ...
  var openWidgetParamEdit = function(name, element) {
    ...
    var url = '<?php echo $this->url(array('action' => 'widget', 'controller' => 'content', 'module' => 'sesnewsletter'), 'admin_default', true)?>';
  ...
  var saveCurrentPageInfo = function(formElement) {
    var url = '<?php echo $this->url(array('action' => 'save', 'controller' => 'content', 'module' => 'sesnewsletter'), 'admin_default', true)?>';
  ...
  var toggleGlobalLayout = function(element) {
    ...
    if( currentLayout == 'default' || currentLayout == '' ) {
      ...
      headerContainer.getElement('a').set('html', '(<?php echo $this->string()->escapeJavascript($this->translate('show on this template')) ?>)');
      footerContainer.getElement('a').set('html', '(<?php echo $this->string()->escapeJavascript($this->translate('show on this template')) ?>)');
    ...
    else
    {
      ...
      headerContainer.getElement('a').set('html', '(<?php echo $this->string()->escapeJavascript($this->translate('hide on this template')) ?>)');
      footerContainer.getElement('a').set('html', '(<?php echo $this->string()->escapeJavascript($this->translate('hide on this template')) ?>)');
  ...
  var deleteCurrentPage = function() {
    ...
    if( !confirm('<?php echo $this->string()->escapeJavascript($this->translate("Are you sure you want to delete this template?")) ?>') ) {
      return false;
    }

    var redirectUrl = '<?php echo $this->url(array()) ?>';
    var url = '<?php echo $this->url(array('action' => 'delete', 'controller' => 'content', 'module' => 'sesnewsletter'), 'admin_default', true)?>';
    var request = new Request.JSON({
      'url' : url,
      'data' : {
        'format' : 'json',
...
<h2><?php echo $this->translate('Manage Templates'); ?></h2>
<p>
...
<div id='admin_cms_wrapper'>
  ...
  <div class="admin_layoutbox_menu">
    <ul>
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_openpage">
          ...
          <ul class="admin_layoutbox_menu_pages">
            <?php foreach( $this->pageListAssoc as $pageType => $pageListSub ): ?>
              <?php foreach( $pageListSub as $page ): ?>
                <li onclick="loadPage(<?php echo $page->template_id ?>);">
        ...
        <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), $(this));">
          <span><?php echo $this->translate("Editing:"); ?></span>
            <?php echo $this->templateObject->displayname ?><span class="more">&nbsp;</span>
        </a>
      </li>
      ...
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_newpage">
        <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_makepage_wrapper" id="admin_layoutbox_menu_makepage_wrapper">
          <div class="admin_layoutbox_menu_makepage">
            <form class="admin_layoutbox_menu_makepage_form" action="<?php echo $this->url(array('action' => 'create')) ?>" method="post">
              <div>
                <?php echo $this->translate('Give your new page a unique name. This will appear in the list of pages in the "Open Template" tab.') ?>
              </div>
              <div class="admin_layoutbox_menu_makepage_form_name">
                <span><?php echo $this->translate("Template Name") ?></span>
                <?php echo $this->formText('name', '', array('id' => 'admin_layoutbox_menu_makepage_form_input', 'maxlength' => '50')) ?>
                <span><?php echo $this->translate("Duplicate Existing Template?") ?></span>
                  ...
                  <?php foreach( $this->pageListAssoc as $pageType => $pageListSub ): ?>
                    <?php foreach( $pageListSub as $page ): ?>
                      <?php if( $page->displayname != "Site Header" && $page->displayname != "Site Footer"): ?>
                        <option value="<?php echo $page->template_id ?>"><?php echo $page->displayname ?></option>
                      <?php endif; ?>
                    <?php endforeach; ?>
                  <?php endforeach; ?>
              </select>
              ...
              <div>
                <button type="submit"><?php echo $this->translate("Create Template") ?></button>
        ...
        <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), $(this));">
          <?php echo $this->translate("New Template") ?>
        </a>
      </li>
    </ul>
  </div>
  ...
  <div class="admin_layoutbox_wrapper">
    ...
    <div class="admin_layoutbox_sub_menu">
      <h3>
        <?php echo $this->translate('Template Block Placement') ?>
      </h3>
      <ul>
        
        <?php if( substr($this->templateObject->name, 0, 6) !== 'header' && substr($this->templateObject->name, 0, 6) !== 'footer'): ?>
        <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_pageinfo">
          ...
          <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), $(this));"><?php echo $this->translate("Edit Template Info") ?></a>
        </li>
          ...
          <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_columnchoices_wrapper" id="admin_layoutbox_menu_columnchoices_wrapper">
            <div class="admin_layoutbox_menu_columnchoices">
              <div class="admin_layoutbox_menu_columnchoices_instructions">
                <?php echo $this->translate("Select a new column layout for this template.") ?>
              </div>
    ...
    <?php // Normal editing ?>
    <?php if( substr($this->templateObject->name, 0, 6) !== 'header' && substr($this->templateObject->name, 0, 6) !== 'footer' && ($this->templateObject->layout == 'default' || $this->templateObject->layout == 'default-simple' || $this->templateObject->layout == '')): ?>

      <div class='admin_layoutbox'>
        <div class='admin_layoutbox_header<?php echo ( empty($this->templateObject->layout) || $this->templateObject->layout == 'default' ? '' : ' admin_layoutbox_header_hidden' ) ?>'>
          <span>
            <?php echo $this->translate('Template Header') ?>
            <span>
              <a href="javascript:void(0);" onclick="toggleGlobalLayout($(this).getParent('div.admin_layoutbox_header'));">
                <?php echo ( empty($this->templateObject->layout) || $this->templateObject->layout == 'default' ? "({$this->translate('hide on this template')})" : "({$this->translate('show on this template')})" ) ?>
              </a>
            </span>
          </span>
        </div>
        ...
        <div class='admin_layoutbox_footer<?php echo ( empty($this->templateObject->layout) || $this->templateObject->layout == 'default' ? '' : ' admin_layoutbox_footer_hidden' ) ?>'>
          <span>
            <?php echo $this->translate('Template Footer') ?>
            <span>
              <a href="javascript:void(0);" onclick="toggleGlobalLayout($(this).getParent('div.admin_layoutbox_footer'));">
                <?php echo ( empty($this->templateObject->layout) || $this->templateObject->layout == 'default' ? "({$this->translate('hide on this page')})" : "({$this->translate('show on this page')})" ) ?>
              </a>
            </span>
          </span>
        </div>
    ...
    <?php elseif( (substr($this->templateObject->name, 0, 6) == 'header' || substr($this->templateObject->name, 0, 6) == 'footer') && ($this->templateObject->layout == 'default' || $this->templateObject->layout == 'default-simple' || $this->templateObject->layout == '')): ?>

      <div class='admin_layoutbox'>
        <?php if( substr($this->templateObject->name, 0, 6) == 'footer' ): ?>
        ...
        <?php if( substr($this->templateObject->name, 0, 6) == 'header' ): ?>
