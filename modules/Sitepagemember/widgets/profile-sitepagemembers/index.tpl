...
<div id="id_<?php echo $this->identity; ?>">
  ...
  <?php endif; ?>
  <?php endif; ?>
  <h2><b>Organization Members</b></h2><br>
  <div id='sitepagemember_search_<?php echo $this->identity ?>' class="sitepagemember_profile_list">
    <?php if (count($this->paginator) > 0): ?>
          <!-- Customised code for members-->
             <ul>
              <?php foreach ($this->test as $sitepagemember): ?>

              <?php $isPageAdmin = $this->sitepage->isPageAdmin($sitepagemember->user_id); ?>
              <?php

               if($sitepagemember->user_id) {
                $userTable = Engine_Api::_()->getItemTable('user');
                $userTableName = $userTable->info('name');
                $email= $userTable->select()->from($userTableName, 'email')->where('user_id =?',$sitepagemember->user_id)->query()->fetchColumn();
                }
              ...
              <div class="sitepagemember_profile_list_info">
                <div class="sitepagemember_profile_list_title" id="sitepage_profile_list_title_<?php echo $sitepagemember->member_id ?>">
                  ...
                  $userTableName = $userTable->info('name');
                  $email= $userTable->select()->from($userTableName, 'email')->where('user_id =?',$sitepagemember->user_id)->query()->fetchColumn();
                  $displayname= $userTable->select()->from($userTableName, 'displayname')->where('user_id =?',$sitepagemember->user_id)->query()->fetchColumn();
                  }

                  ?>
                  <p style="word-break: break-word"><?php echo $displayname;?></p>
                  <p><?php echo $email;?></p>
   
                  <?php if ($sitepagemember->user_id ==0) : ?>
                  <p><?php echo $sitepagemember->email;?></p>
                  <?php endif; ?>
                </div>
          ...
          <!--   <ul>
                 <?php foreach ($this->inviteMembers as $sitepagemember): ?>
                    <?php $isPageAdmin = $this->sitepage->isPageAdmin($sitepagemember->user_id); ?>
                   ...
                   <div class="sitepagemember_profile_list_info">
                       <div class="sitepagemember_profile_list_title" id="sitepage_profile_list_title_<?php echo $sitepagemember->member_id ?>">
                          <p><?php echo $sitepagemember->member_email;?></p>
                          <p>External member</p>
                       </div>
                   </div>
...
<script type="text/javascript">
  ...
  if (MemberTabIdCurrent == MembertabId) {
    ...
    if($(globalContentElement).getElement('.layout_sitepagemember_profile_sitepagemembers_<?php echo $this->identity; ?>')) {
      $(globalContentElement).getElement('.layout_sitepagemember_profile_sitepagemembers_<?php echo $this->identity; ?>').style.display = 'none';
    }
  }
  // });
  ...
  $$('.tab_<?php echo $this->identity; ?>').addEvent('click', function() {
    $(globalContentElement).getElement('.layout_sitepagemember_profile_sitepagemembers_<?php echo $this->identity; ?>').style.display = 'block';
    if(page_showtitle != 0) {
      if($('profile_status') && show_widgets == 1) {
        $('profile_status').innerHTML = "<h2><?php echo $this->string()->escapeJavascript($this->sitepage->getTitle()) ?><?php echo $this->translate(' &raquo; '); ?><?php echo $this->translate('Members'); ?></h2>";
      }
...
<style>
  .sitepage_list_filters.sitepagemember_profile_filters {
    display: none;
  }
  .external_members_list {

  }
  a.buttonlink.icon_sitepage_ad_member.smoothbox ,a.sitepage_gutter_messageowner.buttonlink.smoothbox{
    font-size: 15px;
  }
  div#sitepagemember_search_ {
    margin-top: 8px;
  }
  .sitepagemember_profile_list_options {
    width: 147px !important;
  }
  a.smoothbox.buttonlink.icon_sitepage_highlighted , a.icon_sitepages_edit.smoothbox.buttonlink,
  a.buttonlink.icon_sitepage_unhighlighted.smoothbox,a#remove_page_admin,a#make_page_admin,
  a.buttonlink.smoothbox.icon_sitepage_notification{
    display: none;
  }
  .sitepagemember_profile_list_stats {
    display: none;
  }
