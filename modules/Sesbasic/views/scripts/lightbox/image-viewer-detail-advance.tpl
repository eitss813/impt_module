  ...
  <div id="heightOfImageViewerContent">
    <div id="flexcroll" >
      <div class="ses_pswp_info" id="ses_pswp_info">
        ...
        <div class="ses_pswp_item_title" id="ses_title_get"> <?php echo nl2br($this->photo->getTitle()); ?></div>
        <div class="ses_pswp_item_description" id="ses_title_description"><?php echo nl2br($this->photo->getDescription()) ?></div>
        
        <?php if($this->canEdit) { ?>
          <div class="ses_pswp_item_edit_link"> <a id="editDetailsLink" href="javascript:void(0)" class="sesbasic_button"> <i class="fa fa-edit sesbasic_text_light"></i> <?php echo $this->translate('Edit Details'); ?> </a> </div>
        <?php } ?>
      </div>
      <?php if($this->canEdit) { ?>
...
<div class="pswp__top-bar" style="display:none" id="imageViewerId"> 
  ...
  <a <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum.show.information', 0)) { ?> title="<?php echo $this->translate('Close'); ?>" <?php } else { ?> title="<?php echo $this->translate('Show Info'); ?>" <?php } ?> id="pswp__button--info-show" class="pswp__button pswp__button--info pswp__button--info-show <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum.show.information', 0)) { ?> active <?php } ?>"></a>
  ...
  <div class="pswp__top-bar-action">
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0) { ?>
      <div class="pswp__top-bar-msg pswp__top-bar-btns">
        ...
        <?php if($this->moduleName == 'emultlist' && isset($this->photo->favourite_count)) { echo "sf";die; ?>
          <?php if($this->photo->getType() == 'emultlist_photo') { ?>
            <?php $albumFavStatus = Engine_Api::_()->getDbtable('favourites', 'emultlist')->isFavourite(array('resource_type' => $this->photo->getType(), 'resource_id' => $this->photo->getIdentity())); ?>
