 ...
 */
?>
<?php if(isset($this->photo->owner_id)) { ?>
  <?php $user_id = $this->photo->owner_id; ?>
<?php } else if(isset($this->photo->user_id)) { ?>
  <?php $user_id = $this->photo->user_id; ?>
<?php } ?>
...
<div class="ses_media_lightbox_left">
  ...
  <div class="ses_media_lightbox_options">
    <div class="ses_media_lightbox_options_owner">
    	<?php $albumUserDetails = Engine_Api::_()->user()->getUser($user_id); ?>  
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
      <div class="ses_media_lightbox_options_btns">
        ...
        <?php if(isset($this->photo->favourite_count)){ ?>
          <?php if($this->photo->getType() == 'emultlist_photo' && $this->moduleName == 'emultlist' && Engine_Api::_()->getApi('settings', 'core')->getSetting('emultlist.enable.favourite', 1)) { ?>
            <?php $albumFavStatus = Engine_Api::_()->getDbtable('favourites', 'emultlist')->isFavourite(array('resource_type'=>$this->photo->getType(),'resource_id'=>$this->photo->photo_id)); ?>
            <a href="javascript:;" data-url='<?php echo $this->photo->photo_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn emultlist_favourite <?php echo ($favStatus) ? 'button_active' : '' ; ?>">
              <i class="fa fa-heart"></i>
              <span><?php echo $this->photo->favourite_count; ?></span>
            </a>
          <?php } else if($this->photo->getType() == 'sespage_photo') { ?>
...
<div id="heightOfImageViewerContent">
  <div id="flexcroll" >
    <div class="ses_media_lightbox_media_info" id="ses_media_lightbox_media_info">
      ...
      <div class="ses_media_lightbox_item_title" id="ses_title_get"> <?php echo nl2br($this->photo->getTitle()); ?></div>
      <div class="ses_media_lightbox_item_description" id="ses_title_description"><?php echo nl2br($this->photo->getDescription()) ?></div>
      
      <?php if($this->viewer()->getIdentity() == $user_id) { ?>
...
<a href="javascript:;" class="cross ses_media_lightbox_close_btn exit_lightbox"><i class="fas fa-times sesbasic_text_light"></i></a>
<a href="javascript:;" class="ses_media_lightbox_close_btn exit_fullscreen" title="<?php echo $this->translate('Exit Fullscreen'); ?>" onclick="toogle()"><i class="fas fa-times sesbasic_text_light"></i></a>
