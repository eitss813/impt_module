...
<?php 
	...
	if (empty($results)) {
		return '<div class="global_form"><div><div><p style="color:red;">The required SocialNetworking.Solutions (SNS) Basic Required Plugin is not installed on your website. Please download the latest version of this FREE plugin from <a href="http://www.socialenginesolutions.com" target="_blank">SocialEngineSolutions.com</a> website.</p></div></div></div>';
	} else {
		if (isset($results->enabled) && !empty($results->enabled)) {
			if (empty($finalVersion)) {
				return '<div class="global_form"><div><div><p style="color:red;">The latest version of the SocialNetworking.Solutions (SNS) Basic Required Plugin installed on your website is less than the minimum required version: ' . $plugin_currentversion . '. Please upgrade this Free plugin to its latest version after downloading the latest version of this plugin from <a href="http://www.socialenginesolutions.com" target="_blank">SocialEngineSolutions.com</a> website.</p></div></div></div>';
			} else {
				return '1';
			}
		} else {
			return '<div class="global_form"><div><div><p style="color:red;">The SocialNetworking.Solutions (SNS) Basic Required Plugin is installed but not enabled on your website. So, please first enable it from the "Manage" >> "Packages & Plugins" section.</p></div></div></div>';
		}
	}
