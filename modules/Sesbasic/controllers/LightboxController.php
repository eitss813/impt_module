...
class Sesbasic_LightboxController extends Core_Controller_Action_Standard {
  ...
  public function imageViewerDetailAction() {
    ...
    $this->view->moduleName = $getParams['module'];
    ...
    if(!$album->authorization()->isAllowed($viewer, 'view')){
        ...
        $this->view->imagePrivateURL = Engine_Api::_()->sesbasic()->getFileUrl($imagePrivateURL);
    }

		$this->view->canComment = $album->authorization()->isAllowed($viewer, 'comment');

    /* Insert data for recently viewed widget */
    if( isset($getModule) && in_array($getModule['module'], array('sesalbum', 'album'))) {
    ...
    if($getParams['module'] == 'emultlist') {
      $listing = Engine_Api::_()->getItem('listing', $album->listing_id);
      $this->view->canEdit = $canEdit = $listing->authorization()->isAllowed($viewer, 'edit_'.$listing->type_id);
      $this->view->canDelete = $canDelete = $listing->authorization()->isAllowed($viewer, 'delete_'.$listing->type_id);
			$this->view->canComment = $canComment = $listing->authorization()->isAllowed($viewer, 'comment');
			$this->view->canTag = $canTag = $listing->authorization()->isAllowed($viewer, 'tag_'.$listing->type_id);
    } else {
