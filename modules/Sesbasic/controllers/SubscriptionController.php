 ...
 * @license    http://www.socialengine.com/license/
 */
class Sesbasic_SubscriptionController extends Core_Controller_Action_Standard
{
  /**
   * @var User_Model_User
   */
  ...
  public function gatewayAction()
  {
    ...
    // For Coupon 
    $this->view->couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$subscription->getType().'-'.$subscription->subscription_id.'-1';
  ...
  public function processAction()
  {
    ...
    if( !$gatewayId ||
        !($gateway = Engine_Api::_()->getItem('sesbasic_gateway', $gatewayId)) ||
        !($gateway->enabled) ) {
      return $this->_helper->redirector->gotoRoute(array('action' => 'gateway'));
    }
  ...
  public function returnAction()
  {
    ...
    if( !$this->_user ||
        ...
        !($package = $subscription->getPackage()) ||
        !($gateway = Engine_Api::_()->getItem('sesbasic_gateway', $order->gateway_id)) ) {
      return $this->_helper->redirector->gotoRoute(array(), 'default', true);
    }
    // Get gateway plugin
    $this->view->gatewayPlugin = $gatewayPlugin = $gateway->getGateway();
