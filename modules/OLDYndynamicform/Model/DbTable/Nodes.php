 ...
 */
class Yndynamicform_Model_DbTable_Nodes extends Engine_Db_Table
{
    protected $_rowClass = 'Yndynamicform_Model_Node';
    ...
    /**
     * @return Yndynamicform_Model_Node
     */
     ...
     * @param  $parent_id
     * @return Yndynamicform_Model_Node|NULL
     */
     ...
     * @param   Yndynamicform_Model_Node  $node
     * @param   Array                $data
     * @return  Yndynamicform_Model_Node
     * @throw Exception
     */
    public function insertAfter(Yndynamicform_Model_Node $node, $data) {
        if($node -> pleft == 1) {
     ...
     * @throw Exception
     */
    public function insertBefore(Yndynamicform_Model_Node$node, $data) {
        if($node -> pleft == 1) {
            throw new Exception("can not insert after root");
        }
     ...
     * @throw Exception
     */
    public function appendChild(Yndynamicform_Model_Node $node, $data) {
     ...
     * @throw Exception
     */
    public function prependChild(Yndynamicform_Model_Node$node, $data) {
        $newNode = $this -> fetchNew();
        $newNode -> setFromArray($data);
        $anchor = $node -> pleft;
        $newNode -> pleft = $anchor + 1;
     ...
     * @throw Exception
     */
    public function addChild(Yndynamicform_Model_Node $node, $data) {
        if(!is_object($node)) {
            $node = $this -> getRoot();
        }
    ...
    public function getDescendent($node, $include_own =true) {
        if(!$node instanceof Yndynamicform_Model_Node) {
