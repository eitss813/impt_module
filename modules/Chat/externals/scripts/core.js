    ...
    ChatHandler_Whispers_UI_Abstract = new Class({
        ...
        render : function() {
            ...
            if( this.options.showHide ) {
                ...
                //Close Chat
                if( uid  != 'friends') {
                  this.elements.menuRemoveLink = new Element('a', {
                    'href' : 'javascript:void(0);',
                  ...
                  if( uid ) this.elements.menuRemoveLink.set('id', 'im_menu_' + uid + '_removelink');
                }
...
'
    ...
    ChatHandler_Whispers_UI_Conversation = new Class({
        ...
        render : function() {
            ...
            if( this.handler.handler._supportsContentEditable() ) {
                ...
                this.elements.menuDiv = new Element('div', {
                    'class' : 'im_menu_emotions_wrapper',
                }).inject(this.elements.menuFooter);
                if( this.options.uid ) this.elements.menuDiv.set('id', 'im_menu_' + this.options.uid + '_div');

                this.elements.menuSpan = new Element('span', {
                    'class' : 'im_menu_emotions im_menu_'  + this.options.name + '_emotions',
                    'onmousedown' : 'setChatEmoticonsBoard("'+this.options.uid+'")',
                }).inject(this.elements.menuDiv);
                if( this.options.uid ) this.elements.menuSpan.set('id', 'im_menu_' + this.options.uid + '_span');
            }
    ...
    ChatHandler_Utility = {
        ...
        smilies : $H(chatEmotions),

        replaceSmilies : function(text) {
            this.smilies.each(function(name, val) {
                if( text.indexOf(val) < 0 ) return;
                var parts = text.split(val);
                var image = '<img class="emoticon_img" src="' + this.imageSpec.replace(':name:', name).escapeQuotes() + '" alt="' + val.escapeQuotes() + '" />';
...
function setChatEmoticonsBoard(id) {
    ...
    if($('chat-emoticons-box'))
        $('chat-emoticons-box').destroy();
    ...
    $('im_menu_' + id + '_div').innerHTML = $('im_menu_' + id + '_div').innerHTML + chatString + emotionsString + '</span>';
    $('chat-emoticons-box').toggleClass('emoticons_active');
    $('chat-emoticons-box').toggleClass('');
    $('chat-emoticons-box').toggleClass('chat_emoticons_box_opened');
    $('chat-emoticons-box').toggleClass('chat_emoticons_box_closed');

}


function addChatEmoticonIcon(event) {
    var chat_content = unescape($('im_menu_'+event.getAttribute("data-id")+'_input').value);
    chat_content = chat_content.replace(/&nbsp;/g, ' ');
    $('im_menu_'+event.getAttribute("data-id")+'_input').value = chat_content + ' ' + event.getAttribute("data-code") + ' ';
}
...
function hideChatEmoticonsBoxEvent() {
    if ($('chat-emoticons-box')) {
        $('chat-emoticons-box').removeClass('chat_emoticons_box_opened').addClass('chat_emoticons_box_closed');
    }
}
