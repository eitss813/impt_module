 ...
 * @version    $Id: Fields.php 2015-12-31 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesmultipleform_Form_Custom_Fields extends Engine_Form {
  /* Custom */
  ...
  public function generate() {
    ...
    foreach ($struct as $fskey => $map) {
      ...
      $key = $map->getKey().'_'.$this->getWidget();
      ...
      if (method_exists($element, 'setFieldMeta')) {
        $element->setFieldMeta($field);
			...
			if($map->child_id == 1 && $map->option_id == 0 && $map->field_id == 0)
				$classHide = 'profile_type_'.$this->getWidget();
			else
				$classHide = '';
      // Set attributes for hiding/showing fields using javscript
      $classes = 'field_container field_' . $map->child_id . ' option_' . $map->option_id . ' parent_' . $map->field_id . ' widget_' . $this->getWidget().' '.$classHide;
    ...
    if (!empty($this->_topLevelId))
      $selected[$this->_topLevelId] = $this->_topLevelValue;
		if(isset($_POST['category_id']) && $_POST['category_id'] != 0){
			$category_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesmultipleform')->getMapId($_POST['category_id']);
		...
		if(isset($_POST['subcat_id']) && $_POST['subcat_id'] != 0){
			$subcategory_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesmultipleform')->getSubCatMapId($_POST['subcat_id']);
		...
		if(isset($_POST['subsubcat_id'])  && $_POST['subsubcat_id'] != 0){
			$subsubcategory_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesmultipleform')->getSubSubCatMapId($_POST['subsubcat_id']);
