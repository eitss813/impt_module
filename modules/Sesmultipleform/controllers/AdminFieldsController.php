 ...
 * @version    $Id: AdminFieldsController.php 2015-12-31 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesmultipleform_AdminFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesmultipleform_entry';

  public function indexAction() {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesmultipleform_admin_main', array(), 'sesmultipleform_admin_main_fields');
    parent::indexAction();
  }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesmultipleform_Form_Admin_Settings_Type();
    ...
    if ($form->getValue('duplicate') != 'null' && $form->getValue('duplicate') ) {
      ...
      $field_map_array = $db->select()
              ->from('engine4_sesmultipleform_entry_fields_maps')
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesmultipleform_entry_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesmultipleform_entry_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesmultipleform_entry_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbtable('categories', 'sesmultipleform')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
